...
function create_agenda_links(calendar_output)
  ...
  for key, value in pairs(calendar_output) do
    ...
    agenda_links:add(
      create_link(value.start_date, value.end_date, htmlEntities.encode(value.description), value.video or value.link)
    )
  end
end

local calendar_email_widget = wibox.widget({
  align = "center",
  valign = "top",
  font = "sans italic 12",
  markup = helpers.colorize_text(user.get_active_profile().calendar_email, x.color2),
...
awesome.connect_signal("evil::profile::change_complete", function()
  calendar_email_widget.markup = helpers.colorize_text(user.get_active_profile().calendar_email, x.color2)
end)
