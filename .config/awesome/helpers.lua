...
function helpers.parse_date(date_string)
  return date_string:match("^([0-9]*)-([0-9]*)-([0-9]*)T?([0-9]*):([0-9]*)")
end
...
function helpers.remote_watch(command_or_func, interval, output_file, callback, optional_signal)
  ...
  local run_the_thing = function()
    if type(command_or_func) == "function" then
      ...
      awful.spawn.easy_async_with_shell(command .. " | tee " .. output_file, function(out, err, exitreason, exitcode)
        callback(out, err, exitreason, exitcode)
      end)
    else
